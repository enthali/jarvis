...
- On PM **fix-now** decision: apply requested fixes on the feature branch and
  record outcome in `val-<name>.md` (Verification Report)
- Signal merge-readiness back to PM — **PM performs the merge**, CM does not
...
- **`.jarvis/` is per-installation private** (user decision 2026-05-20). Sessions,
  `heartbeat.yaml`, `autodelivery.json`, `messages.json`, `message-log.json` —
  everything under `.jarvis/` stays local, nothing is versioned. If default configs
  ever need to ship, use a separate mechanism (template in `resources/` + init logic),
  not direct versioning of `.jarvis/`.
- **CM scope is strictly product-changes + feature-branches** (user clarification
  2026-05-20). Repo housekeeping, session-state maintenance, `.gitignore` tweaks,
...
- Sessions live under `.jarvis/sessions/<Name>/` (this folder) — **local-only, not versioned**
- Each session owns its own `context.md` + `session.yaml`; pflege bleibt pro Installation
- Drift from `.github/agents/syspilot.cm.agent.md` is fixed locally (no commit needed)
