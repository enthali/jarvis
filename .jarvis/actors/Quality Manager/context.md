...
CR #54 (notification-agent-mode-reset) — **CLEAR (Round 2, 2026-07-27)**: Round 1's BLOCK (stale `sendPromptToFocusedAgentChat` reference in SPEC_MSG_SENDPROMPT/SPEC_INJ_INJECT) fixed cleanly on first attempt, no mirror-image regression. No open findings remain. PM accepted the 2 non-blocking items (UAT gap, test methodology) as-is.
...
- **Spec code-samples must be reconciled after the CR that wrote them learns the actual implementation shape (2026-07-27, CR #54 R1)**: when a spec deliberately states "the constraint, not the command" and leaves an implementation shape open (e.g. "extra parameter, second exported function, …"), the illustrative code sample from BEFORE that decision was made must still be updated once dev picks a shape — leaving it labeled "unchanged" while the named function was actually renamed/split is not a neutral simplification, it's a factual error the spec's own author introduced within the same CR. Treat "reference implementation" code blocks as an artifact requiring a final reconciliation pass, not as prose exempt from the constraint-vs-command distinction.
