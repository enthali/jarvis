# Change Document: fix-notification-tool-name
...
The auto-delivery notification template shipped with the wrong tool name. The hardcoded default and the spec both reference `jarvis_readMessage`, but agents see the tool as `enthali.jarvis-core/readMessage` (VS Code qualified tool name format). This mismatch caused agents to fail silently and fall back to reading `messages.json` directly instead of using the registered tool. This fix updates the default template in `extension.ts` (both occurrence sites) and in `req_msg.rst` to use the correct qualified name `enthali.jarvis-core/readMessage`. N/A — infrastructure fix; no user story, requirement, or design spec exists for this in the Jarvis spec tree (L0/L1/L2 not applicable).
