# Change Document: release-agent-patch

**Status**: in-progress
**Branch**: feature/release-agent-patch
**Created**: 2026-06-26
**Author**: PM
...
Patch the Jarvis Release Agent to work with our monorepo setup instead of syspilot-specific hardcoded paths. This fixes a blocker where `npm ci` fails after release because package-lock.json is not regenerated when versions are bumped in all 6 package.json files. The fix: (1) read current version from docs/changes/<version>/ directory, (2) bump all 6 package.json files, (3) run npm install to regenerate lockfile. This is a prerequisite for any future releases — without it the release pipeline breaks at step 6 (npm ci).
