# Change Document: dynamic-tree-title

**Status**: in-progress
**Branch**: feature/dynamic-tree-title
**Created**: 2026-07-16
**Author**: PM
...
The Jarvis Entities tree view currently shows a static title "Jarvis Entities" regardless of which workspace/repository is open. In multi-project environments this makes it hard to immediately see which project's entities are displayed. This change makes the tree view title dynamic: it shows the first workspace folder's name (e.g. "my-repo Entities") instead of the generic "Jarvis Entities", falling back to "Jarvis Entities" if no workspace folder is available. The first workspace folder is already the one scanned for `.jarvis` entities, so the title accurately reflects the data source.
