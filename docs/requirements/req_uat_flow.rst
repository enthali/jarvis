...
.. req:: Message Flow Test Data and Harness
   ...
   **Description:**
   ...
   exercise the diagram's data-source edge cases (empty state, 30-entry
   ...
   * AC-1: A test fixture ``testdata/messages/message-log-flow-cap.json``
     ...
     verify the 30-entry cap excludes it. This same fixture SHALL also be
     usable to verify the lens's default window (rank 1–30) and the
     "+30" cap-increase scenario (30 → 60, more entries reachable).
   * AC-2: A test fixture ``testdata/messages/message-log-flow-sample.json``
     ...
     (small dataset, well under 30 entries).
...
.. req:: Message Flow Test Scenarios and Expected Outcomes
   ...
   * AC-3: A test procedure verifies the 30-entry cap excludes older
   ...
   * AC-9: A test procedure verifies the lens's default state on open
     (start = rank 1, end = ``min(loaded total, 30)``).
   ...
   * AC-12: A test procedure verifies the "+30" button increases the data
   ...
   * AC-13: A test procedure verifies the small-dataset edge case — when
     loaded data is well under 30 entries, the lens's default end handle
     matches the loaded total (not a fixed 30) and cannot be dragged past
     it.
