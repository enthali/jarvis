...
.. req:: Modular Install Test Harness & Data
   ...
   * AC-1: An extension-host harness can launch the host with a chosen set of
     ...
     core+mcp — for the acceptance scenarios ``T-1``..``T-9``.
   * AC-2: Zero-trace assertions (``T-2``, ``T-8``) inspect the *static*
     manifest surface (views, settings, commands) plus the runtime tool registry,
     not only runtime visibility.
   ...
   * AC-4: Expected step-by-step outcomes for ``T-1``..``T-9`` are documented in
