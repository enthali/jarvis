...
- **jarvis-syspilot** (GH #39): New optional add-on `enthali.jarvis-syspilot` that manages the syspilot install/update lifecycle. On VS Code startup it fetches the upstream `.github/agents/syspilot.setup.agent.md` from the configured release tag and compares its Frontmatter version to the local copy. On first run (file absent) the file is silently downloaded and the **Syspilot Setup Engineer** actor (auto-created if missing) is always notified. On subsequent runs, the version-match early-return only fires if syspilot is fully installed (`.github/agents/syspilot.pm.agent.md` present); if the installation marker is absent the flow always notifies regardless of version. When a notification is sent, the actor receives a message instructing it to ask the user whether they want to install the update now, skip this version via `jarvis_SyspilotSkipThisVersion()`, or delay by N days via `jarvis_delaySyspilotUpdate(N)`. Both LM tools are registered in the extension manifest (`contributes.languageModelTools`) so they appear in VS Code's "Configure Tools" picker and are selectable by the model. The actor is registered for auto-delivery so the message lands even when its chat tab is not focused. Decision-point logging is emitted to the Jarvis Output Channel at info level. The `jarvis.syspilotUpdate` command forces a re-check. State persists in `.jarvis/syspilot-state.json`; the `jarvis.syspilot.releaseTag` setting controls which upstream ref is tracked (default: `main`).
