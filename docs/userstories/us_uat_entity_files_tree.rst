...
.. story:: Entity File Children Acceptance Tests
   ...
   :status: draft
   :priority: required
   :links: US_EXP_ENTITY_FILES_TREE

   **As a** Jarvis Test Engineer,
   **I want** a set of manual acceptance test scenarios for the entity file
   children tree feature (Session, Project, and Event nodes expanding to show
   ...
   * AC-1: A test verifies that a Project, Event, and Session leaf node each
     become expandable and show their file children on expand (maps to
     ``US_EXP_ENTITY_FILES_TREE`` / T-1, T-2, T-3).
   * AC-2: A test verifies that an entity without a configured ``agent``
     ...
     (maps to ``US_EXP_ENTITY_FILES_TREE`` / T-4).
   ...
   * AC-5: A test verifies fail-open behavior when a configured agent file is
     ...
     no file is auto-created (maps to ``US_EXP_ENTITY_FILES_TREE`` / T-9).
