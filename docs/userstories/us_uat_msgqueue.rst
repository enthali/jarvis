...
.. story:: Message Queue and Send-to-Chat Acceptance Tests
   ...
   * AC-7: Test scenarios verify that the deprecated ``jarvis_sendToSession`` and
     ...
     **hard-deprecated** — every invocation fails with the exact "no longer
     functional" error text and produces zero side effects (no message queued,
     no message popped, no tree refresh)
   * AC-8: A test scenario verifies that each deprecated tool's
     ``modelDescription`` is prefixed with its "DEPRECATED AND DISABLED"
     notice, visible at tool-discovery time before any invocation is attempted
   ...
   **T-11 — Deprecated jarvis_sendToSession is hard-deprecated (error only)**
     ...
     ``text: "Hello from T-11"`` (``senderSession`` provided or omitted — input
     is irrelevant, the handler short-circuits before validation).
     Expected: Call throws an error with exactly: ``This tool is deprecated
     and no longer functional. Use jarvis_sendMessage instead.``; no message
     is appended to the queue; Messages tree does not refresh.

   **T-12 — Deprecated jarvis_readMessage is hard-deprecated (error only)**
     Setup: One or more messages queued for "TestTarget".
     Action: Call ``jarvis_readMessage`` with ``destination: "TestTarget"``.
     Expected: Call throws an error with exactly: ``This tool is deprecated
     and no longer functional. Use jarvis_receiveMessage instead.``; no
     message is popped from the queue (queue contents unchanged); Messages
     tree does not refresh.

   **T-13 — Deprecated tools show DEPRECATED AND DISABLED notice at discovery time**
     Setup: Open the model's tool picker / tool list (e.g. ask the assistant
     to list available Jarvis tools, or inspect the Copilot Chat tools picker).
     Action: Locate ``jarvis_sendToSession`` and ``jarvis_readMessage`` in the
     list.
     Expected: Each tool's description is prefixed with
     ``[DEPRECATED AND DISABLED — use jarvis_sendMessage instead.]`` /
     ``[DEPRECATED AND DISABLED — use jarvis_receiveMessage instead.]``
     respectively, visible before any invocation is attempted.

   **Superseded:** the former T-13 ("``jarvis_sendToSession`` active-tab
   fallback bug preserved") from the soft-deprecation design is retired by
   the 2026-07-03 PM hard-deprecation pivot — the handler now short-circuits
   to an error before any sender-resolution logic (including the fallback
   bug) would run, so there is no longer any queuing behavior to observe.
