...
src/                    — Extension source (TypeScript)
  extension.ts          — Activation, commands (new-entity, new-session, filters, rescan, context-actions, agent sessions, category/task rename/delete/refresh, searchProjects/searchEvents with flattenLeaves() helper, enableAutoDelivery/disableAutoDelivery), populateDefaultPaths() for workspace-settings bootstrap, 14 LM+MCP tools via registerDualTool(): sendToSession, readMessage, listSessions, listProjects, registerJob, unregisterJob, jarvis_listJobs, jarvis_setReminder, jarvis_listReminders, jarvis_cancelReminder, jarvis_listSessionEntities (output includes agent, empty string when no binding), jarvis_createSession (accepts optional agent param; validates against user-invocable agents before creation; both gated: registered only when jarvis.sessions.enabled===true at activation; no runtime add/remove per ADR tool-deregistration.md), jarvis_category, jarvis_task; syncRescanJob()+syncCategoryRefreshJob()+syncTaskRefreshJob() heartbeat bridges, 5-second poll loop for auto-delivery (max 1 msg/tick, marks notified:true), shared LogOutputChannel "Jarvis" (structured logging with levels and module tags); module-level helpers: AgentModeEntry interface, readFrontmatterBool() (regex-based frontmatter bool reader, no yaml dep), discoverAgentModes() (scans .github/agents/*.agent.md with user-invocable: true gate), pickAgentMode() (QuickPick returning mode string or undefined on cancel); stable session helpers: openPinnedResource() (vscode.open preview:false), openNewChatEditor() (workbench.action.openChat + fallback), sendPromptToFocusedAgentChat() (openAgent + fallback); openAgentSession: /rename then init prompt from applyTemplate() with jarvis.agentSession.initPromptTemplate setting (disciplined default: kind/name/contextPath, Decision/Finding/Next rules, 2-week gate); when entity.agent is set, workbench.action.chat.open receives { query, mode } — no mode when absent; auto-delivery notification via applyTemplate() with jarvis.messages.notificationTemplate setting (English default, ${count}/${destination}); newSession: creates session folder + rescan + auto-opens agent chat; validateJobDestinations() inline helper used by jarvis_registerJob LM+MCP handlers (rejects job before persistence if any queue step has an invalid destination)
  ...
  yamlScanner.ts        — Convention-file scanner: folder with project.yaml/event.yaml/session.yaml = leaf; EntityEntry gains agent?: string (populated from session.yaml, undefined when absent or non-string); content-change detection; events sorted by datesStart+name, projects by name; no own timer (rescans via heartbeat)
  ...
  sessionLookup.ts      — Session UUID resolver via state.vscdb (sql.js); exact title match (known edge: prefixed/suffixed titles may miss)
  heartbeat.ts          — Heartbeat scheduler (cron dispatch, step executor, status bar, registerJob/unregisterJob for heartbeat.yaml); exports HeartbeatStep, HeartbeatJob, loadJobs(), executeJob(), notifyFailure(), validateLoadedJobs() (load-time queue-step destination checker, fire-and-forget); imports getAllSessions/filterNamedSessions from sessionLookup for both load-time and fire-time validation
...
- **Heartbeat queue-step destination validation**: Three sites share the same resolver (`getAllSessions`+`filterNamedSessions` from `sessionLookup`): load-time warning (fire-and-forget in `validateLoadedJobs`), fire-time soft skip in `executeQueueStep`, and pre-persistence rejection in `jarvis_registerJob`. Load-time and fire-time are warn-only; `registerJob` throws on first invalid destination.
...
- **Agent discovery gate**: `discoverAgentModes()` only returns agents whose `.agent.md` frontmatter contains `user-invocable: true`. Internal orchestration agents (e.g. `syspilot.implement`) are excluded by design. A missing or unreadable `.github/agents/` directory returns an empty list silently.
