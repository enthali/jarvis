...
description: "Git branching strategy and commit conventions for syspilot. develop branch with feature branches, main = releases only. Who may create/commit to which branches. USE FOR: any git operation, branch creation, committing, pushing, merging."
---
...
syspilot uses a permanent `develop` integration branch with short-lived feature branches:
...
gitGraph
   ...
   checkout main
   merge develop id: "v0.2.4" tag: "v0.2.4"
```

**Workflow Sequence:**

1. `@syspilot.design` creates `feature/<name>` from `develop`
...
5. Completed feature branch is squash-merged into `develop`
6. `@syspilot.release` prepares on `develop` (archive, version bump, release notes, validate, commit+push), then squash-merges `develop` into `main`, tags, and back-merges `main` into `develop`

**Key Properties:**

- One branch per change — isolates each change for independent review
- `develop` as integration target — all features merge here
- Squash-merge everywhere — clean history on `develop` and `main`
- Main = releases only — main always equals the latest release
- Tag on main — `v{version}` tags mark published releases
- Back-merge after release — `git checkout develop && git merge main` prevents conflicts on next release
- Conflict guidance — squash-merge conflicts resolve with `-X theirs` (develop wins)

## Branch Permissions

| Agent | May create | May commit to |
|-------|-----------|--------------|
| `@syspilot.release` | (none) | `main` (squash merge from `develop` + tag); `develop` (prep + back-merge) |
...
`develop` is a permanent branch that all feature branches merge into. No agent creates `develop` — it exists permanently.

## Branch Naming Conventions

| Pattern | Created by | Purpose |
|---------|-----------|---------|
| `develop` | (permanent) | Integration branch, all features merge here |
